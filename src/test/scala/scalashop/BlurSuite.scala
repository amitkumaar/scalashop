...
class BlurSuite extends FunSuite {
  ...
  test("find neighbours for a given image of size 5 * 5 and co-ordinates 2,2 and radius 1"){
    val src = new Img(5,5)
    val neighbours: Set[Point] = findNeighboursIter(src,0,0,0)//findNeighbours(0,0,0).filter(withinArea(src.height,src.width,_))
    println(neighbours)
  }
  ...
  test("boxBlurKernel should correctly handle radius 2") {
    val src = new Img(4, 3)

    src(0, 0) = 0; src(1, 0) = 1; src(2, 0) = 2; src(3, 0) = 9
    src(0, 1) = 3; src(1, 1) = 4; src(2, 1) = 5; src(3, 1) = 10
    src(0, 2) = 6; src(1, 2) = 7; src(2, 2) = 8; src(3, 2) = 11
    val neighbours: Set[Point] = findNeighbours(0,0,2).filter(withinArea(src.height,src.width,_))
    println(neighbours)
